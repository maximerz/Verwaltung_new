...
try {
    ...
    // Tabelle für Konditionen-Vorlagen
    $PDO->exec("CREATE TABLE IF NOT EXISTS konditionen_vorlagen (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        beschreibung TEXT,
    ...
    // Spalten für Hinweise und Konditionen in kundensystem hinzufügen
    try {
        $PDO->exec("ALTER TABLE kundensystem ADD COLUMN hinweise TEXT");
    } catch (Exception $e) {
        // Spalte existiert bereits
    }
    try {
        $PDO->exec("ALTER TABLE kundensystem ADD COLUMN konditionen TEXT");
    } catch (Exception $e) {
        // Spalte existiert bereits
    }

    // Spalte für ausgewählte Konditionen in bestellungen
    try {
        $PDO->exec("ALTER TABLE bestellungen ADD COLUMN konditionen TEXT");
    } catch (Exception $e) {
        // Spalte existiert bereits
    }
    ...
    // Dokumentvorlagen
    $PDO->exec("CREATE TABLE IF NOT EXISTS dokument_vorlagen (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        dokument_typ TEXT UNIQUE NOT NULL,
        name TEXT NOT NULL,
        header_title TEXT,
        firmenname TEXT,
        firmenadresse TEXT,
        ...
        payment_info TEXT,
        footer_text TEXT,
        primary_color TEXT DEFAULT '#5B7DB1',
