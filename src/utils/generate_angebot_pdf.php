...
$template = get_document_template($PDO, 'angebot');
$template_vars = [
    'kunde_name' => trim(($kunde['vorname'] ?? '') . ' ' . ($kunde['nachname'] ?? '')),
    'dokument_nummer' => $order['angebotsnummer'],
    'datum' => date('d.m.Y'),
    'faelligkeit' => '',
];
?>
...
<head>
    ...
    <style>
        ...
        .header {
            text-align: center;
            margin-bottom: 30px;
            border-bottom: 2px solid <?= htmlspecialchars($template['primary_color']) ?>;
            padding-bottom: 20px;
        }
        ...
        .btn-primary {
            background: <?= htmlspecialchars($template['primary_color']) ?>;
            color: white;
        }
        .btn-success {
            background: #28a745;
...
<body>
    <div class="angebot-container">
        ...
        <div class="angebot-details">
            ...
            <?php if (!empty($order['konditionen'])): ?>
            <p><strong>Konditionen:</strong> <?php echo htmlspecialchars($order['konditionen']); ?></p>
            <?php endif; ?>
        </div>
